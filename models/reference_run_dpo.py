...
class ActionSequenceModel(nn.Module):
    ...
    def forward(self, obs: torch.Tensor) -> Tuple[torch.Tensor]:
        ...
        #######################################################
        #########   YOUR CODE HERE - 3-9 lines.    ############
        mean, log_stds = torch.split(
            net_out, self.segment_len * self.action_dim, dim=-1
        )
        mean = mean.reshape(batch_size, self.segment_len, self.action_dim)
        mean = torch.tanh(mean)
        log_stds = log_stds.reshape(batch_size, self.segment_len, self.action_dim)
        log_stds = torch.clamp(log_stds, min=LOGSTD_MIN, max=LOGSTD_MAX)
    ...
    def act(self, obs: np.ndarray) -> np.ndarray:
        ...
        distribution = self.distribution(np.expand_dims(obs, 0))
        sample = distribution.sample().squeeze()
        sample = torch.clamp(sample[0], min=-1, max=1)
...
class DPO(ActionSequenceModel):
    ...
    def update(
        ...
        w_prob = self.distribution(obs).log_prob(actions_w)
        l_prob = self.distribution(obs).log_prob(actions_l)
        with torch.no_grad():
            ref_w_prob = ref_policy.distribution(obs).log_prob(actions_w)
            ref_l_prob = ref_policy.distribution(obs).log_prob(actions_l)

        sigmoid_input = self.beta * (w_prob - ref_w_prob) - self.beta * (
            l_prob - ref_l_prob
        )
        loss = -torch.mean(F.logsigmoid(sigmoid_input))
        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()
